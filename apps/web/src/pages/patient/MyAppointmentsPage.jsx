...
function ReviewModal({ appt, onClose, onDone, t }) {
  ...
  const submit = async () => {
    if (!rating) { setError(t('myAppts.review.selectRating')); return; }
    setLoading(true);
    try {
      await submitReview(appt._id, rating, comment.trim());
      onDone();
    } catch (e) {
      setError(e.response?.data?.message || t('myAppts.review.error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 28, width: 420, maxWidth: '90vw' }}>
        <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 4 }}>{t('myAppts.review.title')}</div>
        <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 20 }}>{appt.doctorId?.name}</div>
        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 8 }}>{t('myAppts.review.rating')}</div>
        <StarPicker value={rating} onChange={setRating} />
        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', margin: '16px 0 8px' }}>{t('myAppts.review.comment')}</div>
        <textarea
          ...
          placeholder={t('myAppts.review.placeholder')}
          maxLength={1000}
          value={comment}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <Button variant="ghost" style={{ flex: 1 }} onClick={onClose}>{t('myAppts.review.cancel')}</Button>
          <Button style={{ flex: 1 }} onClick={submit} disabled={loading}>
            {loading ? t('myAppts.review.submitting') : t('myAppts.review.submit')}
          </Button>
        </div>
      </div>
    </div>
      ...
      <div style={{ background: 'var(--bg3)', border: `1px solid ${!isPast ? 'rgba(15,227,176,0.25)' : 'var(--border)'}`, borderRadius: 'var(--r-sm)', marginBottom: 8, overflow: 'hidden' }}>
        ...
        {!isPast && a.status === 'confirmed' && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderTop: '1px solid var(--border)', fontSize: 12, color: 'var(--text2)' }}>
            ...
            <button
              onClick={async () => { await toggleReminderOptOut(a._id, !a.remindersDisabled); load(); }}
              style={{ width: 38, height: 20, borderRadius: 10, background: a.remindersDisabled ? 'var(--rose, #f43f5e)' : 'var(--border2, #334155)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background .2s' }}
            >
              <span style={{ position: 'absolute', top: 2, left: a.remindersDisabled ? 20 : 2, width: 16, height: 16, borderRadius: 8, background: '#fff', transition: 'left .2s', display: 'block' }} />
            </button>
          </div>
        )}
      </div>
